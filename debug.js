...
var DebugJS = DebugJS || function() {
  ...
  this.INT_CMD_TBL = [
    ...
    {cmd: 'test', fn: this.cmdTest, desc: 'Manage unit test', help: 'test init|set|count|result [-json]|last|ttlresult|status|verify GOT method EXP|end'},
    ...
    {cmd: 'xb64', fn: this.cmdXB64, desc: 'Encodes/Decodes XB64', help: 'xb64 [-e|-d] -k KEY STR'},
...
DebugJS.prototype = {
  ...
  openFeature: function(ctx, f, subfnc, opt) {
    ...
    switch (f) {
      ...
      case DebugJS.ST_EXT_PANEL:
        ...
        var idx = id;
        if (typeof id == 'string') idx = DebugJS.x.panel.getIdx(id);
        if (idx == undefined) idx = ctx.extActPnlIdx;
        if (idx < 0) idx = 0;
        var p = DebugJS.x.panel.getPanel(idx);
        if (!p || p.hidden) {
          DebugJS._log.e('No such panel: ' + subfnc);
          return false;
        }
  ...
  createFileVwrPanel: function(ctx) {
    ...
    o += '<option value="xb64">XB64</option>';
  ...
  decodeFileVwrData: function() {
    ...
    try {
      var FN = {bsb64: ctx.decodeFileVwrDataB64Ext, xb64: ctx.decodeFileVwrDataB64Ext};
  ...
  decodeFileVwrDataB64Ext: function(ctx, src, mode, scheme) {
    ...
    var encFn = (b64type == 'xb64' ? DebugJS.encodeXB64 : DebugJS.encodeBSB64);
    var toB64Fn = (b64type == 'xb64' ? ctx.b64StoB64 : ctx.bsb64toB64);
    var r, data;
  ...
  onFileLoadedB64: function(ctx, file, dturl) {
    ...
    if (ctx.fileVwrSysCb) {
      ...
      var FN = {bsb64: ctx.b64toBSB64, xb64: ctx.b64toXB64};
      var f = FN[ctx.fileVwrB64Slct.value];
  ...
  b64toXB64: function(b64, k) {
    var b = DebugJS.Base64.decode(b64);
    return DebugJS.encodeXB64(b, k);
  },
  xb64toB64: function(b64, k) {
    var b = DebugJS.decodeXB64(b64, k, 1);
    return DebugJS.Base64.encode(b);
  },
  ...
  cmdXB64: function(arg, tbl, echo) {
    ...
    return DebugJS.ctx.execEncAndDec(arg, tbl, echo, true, DebugJS.encodeXB64, DebugJS.decodeXB64, iIdx, k);
  },
  ...
  cmdTest: function(arg, tbl) {
    ...
    switch (op) {
      ...
      case 'count':
        ...
        break;
      case 'last':
        r = test.getLastResult();
        DebugJS._log(test.getStyledStStr(r));
        break;
      case 'result':
        if (opt == '-json') {
          r = test.getResult(1);
  ...
  cmdUnique: function(arg, tbl) {
    ...
    var strct = DebugJS.hasOpt(arg, 'strict');
    try {
      var a = eval(v);
      var r = DebugJS.arr2set(a, strct);
      if (s) r.sort();
  ...
  initExtPanel: function(ctx) {
    ...
    var pnls = ctx.extPanels;
    if (pnls.length > 0) {
      if (DebugJS.x.panel.len()) DebugJS.showExtBtn(true);
      for (var i = 0; i < pnls.length; i++) {
        var p = pnls[i];
        if (p) {
...
DebugJS.encodeXB64 = function(s, k) {
  var a = ((typeof s == 'string') ? DebugJS.UTF8.toByteArray(s) : s);
  var x = DebugJS.UTF8.toByteArray(k);
  var b = DebugJS._encodeXB64(a, x);
  return DebugJS.Base64.encode(b);
};
DebugJS._encodeXB64 = function(a, k) {
  var ln = a.length;
  var kl = k.length;
...
DebugJS.decodeXB64 = function(s, k, byB) {
  var b = DebugJS.Base64.decode(s);
  var x = DebugJS.UTF8.toByteArray(k);
  var a = DebugJS._decodeXB64(b, x);
  if (!byB) a = DebugJS.UTF8.fromByteArray(a);
  return a;
};
DebugJS._decodeXB64 = function(a, k) {
  var al = a.length;
  var kl = k.length;
...
DebugJS.fmtXml = function(tkns, indt, rmvCmnt) {
  ...
  var lv = 0;
  var x = '';
  var mx = DebugJS.getXmlMixedContent(tkns);
  for (var i = 0; i < tkns.length; i++) {
    ...
    x += tkn;
    if ((tp == DebugJS.XML_TKN_OPN) && (mx[i] != undefined)) {
      var end = mx[i];
      for (var j = i + 1; j <= end; j++) {
        var tp2 = DebugJS.getXmlTknType(tkns[j], DebugJS.XML_TKN_NUL);
        if (rmvCmnt && (tp2 == DebugJS.XML_TKN_CMT)) continue;
        x += tkns[j];
      }
      i = end;
      tp0 = DebugJS.XML_TKN_CLS;
      continue;
    }
    tp0 = tp;
  }
...
DebugJS.getXmlMixedContent = function(tkns) {
  var mx = {};
  var stk = [];
  for (var i = 0; i < tkns.length; i++) {
    var tkn = tkns[i];
    var tp = DebugJS.getXmlTknType(tkn, DebugJS.XML_TKN_NUL);
    var c = stk[stk.length - 1];
    if (tp == DebugJS.XML_TKN_OPN) {
      if (c) c.m = 1;
      stk.push({s: i, t: 0, d: 0, m: 0});
    } else if (tp == DebugJS.XML_TKN_CLS) {
      c = stk.pop();
      if (c && ((c.t && (c.d || c.m)) || (c.d && c.m))) mx[c.s] = i;
    } else if (tp == DebugJS.XML_TKN_CDT) {
      if (c && (tkn.length > 12)) c.d = 1;
    } else if ((tp == DebugJS.XML_TKN_EMP) || (tp == DebugJS.XML_TKN_CMT) || (tp == DebugJS.XML_TKN_PI) || (tp == DebugJS.XML_TKN_DTD) || (tp == DebugJS.XML_TKN_XML)) {
      if (c) c.m = 1;
    } else if (c && /\S/.test(tkn)) c.t = 1;
  }
  return mx;
};
...
DebugJS.bat.exec = function() {
  ...
  if (ctrl.stopReq) {
    DebugJS._log.e('ERROR: SCRIPT STOPPED (L:' + ctrl.pc + ')');
    DebugJS._log.e('--------------------------------');
    for (var i = -3; i <= 0; i++) {
      var pc = ctrl.pc + i;
    ...
    for (i = stk.length - 1; i >= 0; i--) {
      pc = stk[i].lr;
      DebugJS._log.e('at ' + pc + ': ' + DebugJS.trimDownText(bat.cmds[pc - 1], 98));
    }
    ctrl.stopReq = false;
...
DebugJS.test.initData = function() {
  ...
  data.cnt = {pass: 0, fail: 0, error: 0, skip: 0};
  data.results = {};
};
...
DebugJS.test.getCountStr = function(cnt) {
  ...
  return '<span style="color:' + test.COLOR_PASS + '">Pass</span>:' + cnt.pass + '/' + total + ' <span style="color:' + test.COLOR_FAIL + '">Fail</span>:' + cnt.fail + ' <span style="color:' + test.COLOR_ERR + '">Error</span>:' + cnt.error + ' <span style="color:' + test.COLOR_SKIP + '">Skip</span>:' + cnt.skip;
};
DebugJS.test.getSumCount = function() {
  var test = DebugJS.test;
  var cnt = {pass: 0, fail: 0, error: 0, skip: 0};
  var keys = {};
  keys[test.ST_PASS] = 'pass';
  keys[test.ST_FAIL] = 'fail';
  keys[test.ST_ERR] = 'error';
...
DebugJS.test.countTotal = function(cnt) {
  return (cnt.pass + cnt.fail + cnt.error + cnt.skip);
};
...
DebugJS.test.getResult = function(j) {
  var d = DebugJS.test.data;
  var r = {
    name: d.name,
    desc: d.desc,
    startTime: d.startTime,
    endTime: d.endTime,
    totalResult: d.ttlRslt,
    count: d.cnt,
    results: d.results
  };
...
DebugJS.test.getStatus = function() {
  var keys = ['name', 'running', 'startTime', 'endTime', 'seq', 'executingTestId', 'lastRslt', 'ttlRslt', 'cnt'];
  var r = {};
  for (var i = 0; i < keys.length; i++) {
    var k = keys[i];
    r[k] = DebugJS.test.data[k];
  }
  return r;
};
...
DebugJS.x.panel.getIdx = function(nm) {
  var a = DebugJS.ctx.extPanels;
  for (var i = 0; i < a.length; i++) {
...
DebugJS.x.panel.getPanel = function(id) {
  var a = DebugJS.ctx.extPanels;
  var i = (typeof id == 'string' ? DebugJS.x.panel.getIdx(id) : id);
  var p = a[i];
  return (p ? p : null);
};
...
DebugJS.x.panel.isActive = function(nm) {
  if (!(DebugJS.ctx.status & DebugJS.ST_EXT_PANEL)) return false;
  var i = DebugJS.x.panel.getIdx(nm);
...
DebugJS.x.panel.switchPanel = function(id) {
  var i = (typeof id == 'string' ? DebugJS.x.panel.getIdx(id) : id);
