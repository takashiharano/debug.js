...
DebugJS.prototype = {
  ...
  createResizeSideArea: function(cursor, state, width, height) {
    ...
    el.className = ctx.id + '-resize-side';
  ...
  createResizeCornerArea: function(cursor, state) {
    var ctx = DebugJS.ctx;
    var el = document.createElement('div');
    el.className = ctx.id + '-resize-corner';
    el.style.cursor = cursor;
    el.onmousedown = function(e) {
      if (!(ctx.uiStatus & DebugJS.UI_ST_RESIZABLE)) return;
      ctx.startResize(ctx, e);
  ...
  expandDbgWin: function(mode) {
    ...
    if ((mode == 'expand') &&
        (sp.w == DebugJS.DBGWIN_EXPAND_W + border) && (sp.h == DebugJS.DBGWIN_EXPAND_H + border)) {
      return;
    }
    ctx.saveSizeAndPos(ctx);
    switch (mode) {
  ...
  createHtmlSrcPanel: function(ctx) {
    ...
    ctx.htmlSrcUpdateInput = DebugJS.ui.addTextInput(ctx.htmlSrcHeaderPanel, '50px', 'right', UPDATE_COLOR, ctx.htmlSrcUpdateInterval, ctx.onchangeHtmlSrcUpdateInterval);
  ...
  closeTextChecker: function() {
    var ctx = DebugJS.ctx;
    if ((ctx.toolsActiveFnc & DebugJS.TOOLS_FNC_TEXT) && (ctx.txtChkPanel)) {
  ...
  closeFileLoader: function() {
    var ctx = DebugJS.ctx;
    if ((ctx.toolsActiveFnc & DebugJS.TOOLS_FNC_FILE) && (ctx.fileVwrPanel)) {
  ...
  getBinDumpHtml: function(buf, mode, showAddr, showSpace, showAscii) {
    ...
    html += DebugJS.ui.createBtnHtml('[SP]', 'DebugJS.ctx.toggleShowSpace()', (showSpace ? '' : 'color:' + DebugJS.COLOR_INACTIVE)) + ' ';
  ...
  closeHtmlEditor: function() {
    var ctx = DebugJS.ctx;
    if ((ctx.toolsActiveFnc & DebugJS.TOOLS_FNC_HTML) && (ctx.htmlPrevBasePanel)) {
  ...
  cmdTimer: function(arg, tbl) {
    ...
    switch (op) {
      case 'start':
        DebugJS.timeStart(tmrNm);
        break;
      case 'split':
        DebugJS.timeSplit(tmrNm, false);
        break;
      case 'stop':
        DebugJS.timeEnd(tmrNm);
        break;
      case 'list':
        DebugJS.timeList();
        break;
...
DebugJS.addPropSep = function(ctx) {
...
DebugJS.timeStart = function(tmrNm, msg) {
  var _tmrNm = tmrNm;
...
DebugJS.timeGetCount = function(tmrNm) {
...
DebugJS.timePause = function(tmrNm) {
  var now = (new Date()).getTime();
  var ctx = DebugJS.ctx;
...
DebugJS.timeRestart = function(tmrNm) {
  var now = (new Date()).getTime();
  var ctx = DebugJS.ctx;
  if (ctx.timers[tmrNm]) {
...
DebugJS.timeSplit = function(tmrNm, isEnd, msg) {
  var now = (new Date()).getTime();
  var ctx = DebugJS.ctx;
  var _tmrNm = tmrNm;
...
DebugJS.timeReset = function(tmrNm) {
  var now = (new Date()).getTime();
  var ctx = DebugJS.ctx;
...
DebugJS.timeEnd = function(tmrNm, msg) {
  return DebugJS.timeSplit(tmrNm, true, msg);
};
DebugJS.timeLog = function(msg, tmrNm) {
  var now = (new Date()).getTime();
  var ctx = DebugJS.ctx;
...
DebugJS.timeCheck = function(tmrNm, now) {
  if (tmrNm === undefined) tmrNm = DebugJS.DFLT_TIMER_NAME;
...
DebugJS.setStyleIfObjNA = function(obj, exceptFalse) {
  ...
  if ((exceptFalse && ((obj == undefined) || (obj == null))) ||
    ...
    s = '<span class="' + DebugJS.ctx.id + '-na">' + obj + '</span>';
  }
  return s;
};
...
DebugJS.time.start = function(tmrNm, msg) {
  DebugJS.timeStart(tmrNm, msg);
};
DebugJS.time.split = function(tmrNm, msg) {
  var t = DebugJS.timeSplit(tmrNm, false, msg);
  if ((msg === null) || ((tmrNm === null) && (msg === undefined))) {
    return t;
  }
  return;
...
DebugJS.time.end = function(tmrNm, msg) {
  var t = DebugJS.timeEnd(tmrNm, msg);
  if ((msg === null) || ((tmrNm === null) && (msg === undefined))) {
    return t;
  }
  return;
...
DebugJS.time.check = function(tmrNm) {
  return DebugJS.timeCheck(tmrNm, new Date());
};
...
DebugJS.x.addCmdTbl = function(table) {
  ...
  for (var i = 0; i < table.length; i++) {
    var c = table[i];
    if ((ctx.existCmd(c.cmd, ctx.INT_CMD_TBL)) || (ctx.existCmd(c.cmd, ctx.EXT_CMD_TBL))) {
