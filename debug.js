...
DebugJS.prototype = {
  ...
  createPanels: function(ctx) {
    ...
    if (opt.useElementInfo) {
      ctx.elmInfoBtn = ctx.createHeaderBtn('elmInfoBtn', 'DOM', 3, null, ctx.toggleElmInfo, 'status', 'ST_ELM_INFO', 'DOM_BTN_COLOR', false);
    }
    if (opt.useHtmlSrc) {
  ...
  updateSystemTime: function() {
    ...
    var html = '<pre>' + span + 'SYSTEM TIME</span> : ' + DebugJS.convDateTimeStr(DebugJS.getDateTime(time)) + '\n' +
  ...
  showSystemInfo: function(e) {
    ...
    html += DebugJS.addPropSep(ctx);
    html += DebugJS.addSysInfo('css    ', loadedStyles);
    html += DebugJS.addPropSep(ctx);
    html += DebugJS.addSysInfo('script ', loadedScripts);
    html += DebugJS.addPropSep(ctx);
    html += DebugJS.addSysInfo('jQuery ', jq);
    html += DebugJS.addPropSep(ctx);
  ...
  drawStopwatchCu: function() {
    ...
    ctx.timerSwCuLabel.innerHTML = ctx.createTimeStrCu(tm);
  },
  ...
  drawStopwatchCd: function() {
    ...
    ctx.timerSwCdLabel.innerHTML = ctx.createTimeStrCd(tm);
  },
  ...
  onFileLoadedAuto: function(ctx, file, cnt) {
    ...
    if (DebugJS.isBat(cnt) || DebugJS.isB64Bat(cnt)) {
      ...
      ctx.fmtJson(ctx, DebugJS.delAllNL(cnt));
    }
  },
  ...
  _cmdPause: function(op, key, tout) {
    ...
    if (tout > 0) {
      bat.ctrl.pauseTimeout = (new Date()).getTime() + tout;
  ...
  cmdPoint: function(arg, tbl, echo) {
    ...
    if (op == 'init') {
      ...
      ctx._cmdPointCh(ctx, args[1], echo);
    } else {
  ...
  _cmdPointCh: function(ctx, ch, echo) {
    var point = DebugJS.point;
    if (ch == undefined) {
...
DebugJS.bat.restartPauseTmr = function() {
  var bat = DebugJS.bat;
  var t = bat.ctrl.pauseTimeout - (new Date()).getTime();
  if (t > 0) {
...
DebugJS.log.e = function(m) {
  var ctx = DebugJS.ctx;
  if (ctx.status & DebugJS.ST_LOG_SUSPEND) return;
